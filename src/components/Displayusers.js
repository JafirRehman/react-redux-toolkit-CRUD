...
function Displayusers() {
    ...
    return (
        <>
            ...
            <div className="grid-container" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: "50px" }}>
                {pop && <Popup id={id} setPop={setPop} />}
                {ourbelovedusers &&
                    ourbelovedusers.filter(abc=>{
                        if(jafir.requesteddata.length===0){
                            return abc;
                        }else{
                            return abc.name.toLowerCase().includes(jafir.requesteddata.toLowerCase())
                        }
                    })
                    .map((user) => (
                        <div key={user.id} className="card">
                            <div className="card-body">
                                ...
                                <button className="card-link" onClick={() => [setId(user.id), setPop(true)]}>
                                    View
                                </button>
                                <Link className="card-link" to={`/updateuser/${user.id}`}>
                                    Edit
                                </Link>
                                <button className="card-link" onClick={() => dispatch(apideleteuser(user.id))}>
                                    Delete
                                </button>
                            </div>
                        </div>
